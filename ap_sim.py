...
vectorized_messages3 = llm.model.embed_tokens(tokenized_messages3['input_ids'][0]).unsqueeze(0)
...
output_terms_vec0 = llm.model.embed_tokens(output_terms_tok)
...
for t in range(0,24*60*10):
    ...
    #    total_loss = loss_ce
    #else:
    total_loss = errp.max()+(errn*errn).max() #+loss_ce
    total_loss.backward()
    #if total_loss > 0.1:
    optimizer.step()
...
llm.save_pretrained("fuzzy-llm")
tokenizer.save_pretrained("fuzzy-llm")